...
window.onload = function() {
	...
	if (userFavs !== NULL){
		...
		favoriteGistEntries.forEach(function(gistEntries)) {
...
request.onreadystatechange = function()){//Step 1:  Sending request part ii in Getting Started with AJAX Mozilla documentation
	if(this.readyState == 4){//is it ready or not?
		...
		var f;
		for (f=0;f<=gistsForDisplay.length;f++){//TODO
			userFavs.forEach(function())
			if gistsForDisplay[f] === 
		}
		return gistsForDisplay;
		}
	}
...
function listGistEntry(gistEntry, htmlLocation){
	...
	var DomLoc = document.getElementById(htmlLocation);
...
function addFavorite(id) {
	...
	var jsontmp = {'id':gistEntry['id'], 'addr': 'https://api.github.com/gists/'+gistEntry['id'], 'description':} //local storage  via JSON format; adapted from solution and discussion found at http://stackoverflow.com/questions/23728626/localstorage-and-json-stringify-json-parse, particularly the top answer
